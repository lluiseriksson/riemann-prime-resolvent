"""Top-level unconditional localized positivity certificate at a=1/2."""
...
from experiments.theta_pencil.support_05_endpoint_certificate import (
    ...
    certify_support_05_endpoint,
)


@dataclass(frozen=True)
class Support05Certificate:
...
def certify_support_05() -> Support05Certificate:
    """Prove A_(1/2) is strictly positive on both parity sectors."""

    endpoint = certify_support_05_endpoint()
    ...
    if even.temple_lower <= 0 or odd.temple_lower <= 0:
        raise ArithmeticError("the endpoint Kato--Temple bound is not positive")
    return Support05Certificate(
        endpoint=endpoint,
