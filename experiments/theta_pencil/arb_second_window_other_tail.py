"""Certified non-flux/non-singular remainder in the second-window tail."""
...
def certify_second_window_other_tail(
    half_width: float,
    edge_degree: int = 16,
    ...
    Endpoint flux and the adjacent ``R log U`` singular part are excluded:
    their signed Grams are retained separately.  Every entry here is an
    operator-norm bound, so the spectral norm of the seven-by-seven
    comparison matrix bounds their direct-sum map.
    """
