...
### Proposition 4.1a.11 (rigidity of exact prime-lag annihilators)

Put \(L=2a\), \(h=\log 2\), and \(k=\log 3\).  Call a continuous function
\(f\) on an interval \(I\) of length \(L\) an exact prime-lag annihilator if

\[
 f(x+\log n)=f(x)                                         \tag{PA1}
\]

whenever both arguments belong to \(I\), for every active prime power
\(n\le e^L\).  Then the following statements hold.

1. If \(L\ge h+k=\log6\), every exact prime-lag annihilator is constant.
2. If \(L>k\) and \(f\) is real analytic on a neighbourhood of \(I\), it is
   already constant from the two conditions with \(n=2,3\).
3. If

   \[
    \log3<L<2\log3-\log2=\log(9/2),                       \tag{PA2}
   \]

   there are nonconstant smooth exact prime-lag annihilators.  In this range
   the only active prime powers are \(2,3,4\).

#### Proof

Translate \(I\) to \([0,L]\).  The relation with lag \(h\) extends \(f\)
to a continuous \(h\)-periodic function \(F\) on the real line.  If
\(L-k\ge h\), the relation with lag \(k\) holds on a full period of \(F\),
and hence everywhere.  The ratio \(k/h\) is irrational: a rational equality
would imply \(3^q=2^p\) for positive integers \(p,q\).  The subgroup
generated by \(h,k\) is therefore dense, and continuity makes \(F\), hence
\(f\), constant.  This proves the first assertion.

If \(f\) is real analytic and \(L>k>h\), the same \(h\)-periodic extension
is real analytic: the identity at lag \(h\) matches all derivatives across
each seam.  The analytic function \(F(x+k)-F(x)\) vanishes on the nonempty
overlap interval \((0,L-k)\), so it vanishes identically.  Irrationality and
continuity again force constancy, proving the second assertion.

For the last assertion set

\[
 d=L-k,\qquad \alpha=k-h.
\]

Condition (PA2) is exactly \(0<d<\alpha\).  Choose a nonzero smooth
\(h\)-periodic function \(F\) whose support modulo \(h\) lies compactly in
the open interval \((d,\alpha)\), and restrict it to \([0,L]\).  Its
\(h\)- and \(2h\)-differences vanish.  If \(0\le x\le d\), then \(F(x)=0\).
Moreover \(k=h+\alpha\), and the residue of \(x+k\) modulo \(h\) lies either
in \([\alpha,h]\) or in \([0,d]\), so \(F(x+k)=0\) as well.  Thus the lag
\(k\) difference also vanishes on its full overlap.  Finally
\(L<\log(9/2)<\log5\), so no prime power beyond \(2,3,4\) is active.  The
restriction is the required nonconstant smooth annihilator. \(\square\)

For such an annihilator the entire prime line in (GT1) vanishes exactly.
The proposition therefore calibrates, rather than closes, the multiplier
gate.  Exact annihilation is globally trivial once \(a\ge\frac12\log6\), and
it is trivial for analytic multipliers throughout the presently possible
first-crossing range \(a>0.72>\frac12\log3\).  A nonconstant smooth escape
exists at least in

\[
 0.72<a<\frac12\log(9/2)=0.752038\ldots,                  \tag{PA3}
\]
