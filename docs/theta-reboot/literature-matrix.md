...
| [Suzuki, *Weil's quadratic form via the screw function* (2026)](https://arxiv.org/abs/2606.09096) | A continuous-kernel realization of the localized Weil form, its Friedrichs operator \(A_a\), continuity of the lowest eigenvalue, and unconditional small-support positivity. | Excluding a zero crossing of the lowest eigenvalue for every support radius. | **Primary active object.** Use the exact Weil functional and a stable Dirichlet compression; finite positivity remains only a falsifier. |
