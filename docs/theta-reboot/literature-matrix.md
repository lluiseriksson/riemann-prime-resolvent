...
| [Suzuki, *Weil's quadratic form via the screw function* (2026)](https://arxiv.org/abs/2606.09096) | A continuous-kernel realization of the localized Weil form, its Friedrichs operator \(A_a\), continuity of the lowest eigenvalue, and an unconditional \((1,1)\)-deficiency construction whose extension characteristics have only real zeros. | Excluding a zero crossing for every support, or proving convergence of the finite-support Weyl functions without choosing near-zero shifts circularly. | **Two active outputs.** A multiband Schur certificate proves \(A_{0.72}\succeq9.8685\cdot10^{-17}I\), hence \(\lambda_a>0\) for \(0<a\le0.72\). Separately, the analytically type-correct limit object is a ratio of two extension characteristics (a Weyl/Herglotz function), not one entire characteristic converging globally to the meromorphic ratio \(z^2\xi/\xi'\). |
